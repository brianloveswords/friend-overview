...
const proto = create(connection, {
  ...
  selectQuery: function selectQuery(opts, callback) {
    ...
    if (opts.sort || opts.relationships)
      queryString += sortStatement(opts)
      ...
      query.on('result', function onResult(row) {
        ...
        forEach(relationships, function (key, rel) {
          ...
          if (rel.type == 'hasMany') {
            ...
            // console.log('current', current[rel.pivot])
...
function selectWithJoinStatement(table, fields, relationships) {
  ...
  forEach(relationships, function (key, rel) {
    ...
    joinString = joinString +
      ' INNER JOIN '+ escapeId(otherTable) +
      ' ON ' + escapeId([table, key].join('.')) +
      ' = ' + escapeId([otherTable, rel.field].join('.'))
  })
...
function sortStatement(opts) {
  const sorting = []
  const relationships = opts.relationships
  const table = opts.table
  if (relationships) {
    // TODO: explain the whole `pivot` thing, necessary for streaming in
    // a hasMany relationship since we need to know what column to watch
    // for changes so we can emit a new row.
    forEach(relationships, function (key, rel) {
      const pivot = rel.pivot
      if (pivot) {
        const field = escapeId([table, pivot].join('.'))
        sorting.push(field)
      }
    })
  }

...
function populateTweets(callback) {
  ...
  function updateUser(entry, next) {
    ...
    if (lastUpdate + TTL > now ) {
      ...
      console.log('skipping %s (last updated %s)', user.screen_name, isoString)
      return next()
    }

    getStatuses(user.screen_name, function (err, statuses) {
      if (err) return errorCallback(err)
      console.log('processing %s [%s tweets]...', user.screen_name, statuses.length)
